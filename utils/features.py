...
def calculate_logmel(audio_path, sample_rate, feature_extractor):
    # Read audio
    (audio, fs) = read_audio(
        audio_path, max_len=config.audio_samples, target_fs=sample_rate
    )
...
def read_meta_flusense(meta_csv, exclude):
    ...
    for row in df.iterrows():
        file_name = str(row[1]["filename"])
        # FluSense metadata often stores filenames with extension.
        # CovNet later appends ".wav", so we normalize to stem here.
...
def calculate_features(args):
    ...
    if dataset == "flusense":
        ...
        audio_names, labels, intervals, seg_num = read_meta_flusense(
            meta_csv, config.exclude
        )
        ...
        if dataset == "flusense":
            ...
            for audio_name, label, interval in list(
                ...
                if len(audio) <= audio_samples_flusense:
                    ...
                    for seg in range(
                        0,
                        len(audio) - audio_samples_flusense + 1,
                        audio_samples_flusense,
                    ):
                        soundfile.write(
                            file="temp.wav",
                            data=audio[seg : (seg + audio_samples_flusense)],
                            samplerate=fs,
                        )
                        feature = calculate_logmel(
                            audio_path="temp.wav",
                        ...
                        # time.sleep(1)
                    if (
                        len(audio) - (seg + audio_samples_flusense)
                        >= audio_samples_flusense / 2.0
                    ):
                        audio = np.tile(audio[seg + audio_samples_flusense :], 2)[
                            :audio_samples_flusense
                        ]
                        soundfile.write(file="temp.wav", data=audio, samplerate=fs)
                        feature = calculate_logmel(
            ...
            hf.create_dataset(
                name="logmel", shape=(audios_num, frame_num, mel_bins), dtype=np.float32
            )
            hf.create_dataset(name="label", shape=(audios_num,), dtype=np.int16)
            for i, (audio_name, label) in enumerate(
                list(zip(audio_names, covid_statuses))
            ):
                hf["audio_name"][i] = audio_name.encode()
                hf["label"][i] = label
                audio_path = os.path.join(audio_dir, audio_name + ".wav")
                (audio, fs) = read_audio(
                    audio_path, max_len=audio_samples, target_fs=sample_rate
                )
                soundfile.write(file="temp.wav", data=audio, samplerate=fs)
                feature = calculate_logmel(
                    audio_path="temp.wav",
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset", type=str, choices=["flusense", "coughvid", "dicova", "compare"]
    )
